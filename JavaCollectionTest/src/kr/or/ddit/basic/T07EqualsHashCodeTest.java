 ...
 *  함수이다.  해시함수에 의해 얻어지는 값은 해시 값, 해시코드 또는 간단히 해시라고도 한다.
 *  
 ...
 *  HashSet, HashMap, Hashtable 에서는 객체가 같은지 여부는 데이터를 추가할 때
 *  검사한다.
 *  
 ...
 *  4. hashCode()는 기본적으로 힙메모리에 있는 객체에 대한 메모리 주소 매핑정보를 기반으로 
 *   하는 정수값을 반환한다. 그러므로 클래스에서 hashCode()를 override 하지 않으면
 *   절대로 두 객체가 같은 것으로 간주될 수 없다.
 *  5. equal()메서드를 override하는 경우에 hashCode()도 override 해야한다.
 *  
 *  	-hashCode()에서 사용하는 '해싱 알고리즘'에서 서로 다른 객체에 대하여 같은 해시코드 값을
 *  	생성할 수 있다. 그래서 객체가 같지 않더라도 해시코드 값이 같을 수 있다.
 * 
 */
	public static void main(String[] args) {
		...
		System.out.println(new String("이순신").hashCode());
		...
		System.out.println("p1, p1 등록 후 데이터 : ");
		for(Person p : pSet) {
