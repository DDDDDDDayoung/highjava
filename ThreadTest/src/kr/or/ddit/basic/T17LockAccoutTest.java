 ...
 * (Lock 객체를 이용한 동기화 처리)
 */
public class T17LockAccoutTest {
	/*
	   락 기능을 제공하는 클래스
	   
	   - ReentrantLock : Read 및 Write구분 없이 사용하기 위한 락크래스로
	   		동기화 처리를 위해 사용된다. Synchronized를 이용한 동기화 처리보다
	   		부가적인 기능이 제공된다. 
	   				ex) fairness 설정 등. => 가장 오래 기다린 스레드가 가장 먼저 락 획득함.
	   
	   - ReentrantReadWritheLock : Read 및 Write 락을 구분하여 사용가능함.
	   				여러 스레드가 동시에 read 작업은 가능하지만, write작업은 단지
	   				하나의 스레드만 가능함 (exclusive)
	   	 	=> Write 보다 Read 위주의 작업이 많이 발생하는 경우에 사용하면 보다
...
class LockAccount {
	...
	public void deposit (int money) {
		...
		// 동기화의 끝을 나타낸다. lock()메서드로 동기화를 설정한 곳에서는
		// 반드시 unlock()으로 해제해 주어야 한다.
		lock.lock(); // 락 설정
		balance += money;
		lock.unlock(); //락 해제
	...
	public boolean withdraw(int money) {
		...
		try {
			...
			lock.unlock();	//->finally에 넣으면 반드시 해제 가능
